//
//  SandPluginBluetoothAce.m
//  sand-plugin-bluetooth-ace
//
//  Created by Qianjiao Wang on 2022/4/28.
//

#import "SandPluginBluetoothAce.h"
#import "Const.h"

@interface SandPluginBluetoothAce (){
    //蓝牙单例对象
    BabyBluetooth *baby;
    //缓存已经扫描到的蓝牙对象
...
@implementation SandPluginBluetoothAce
            ...
            for(CBCharacteristic *characteristic in characteristics){
                ...
                [arr addObject:@{
                    ...
                    @"properties":@{
                        ...
                        @"writeNoResponse":[NSNumber numberWithBool:characteristic.properties & CBCharacteristicPropertyWriteWithoutResponse],
                        @"signedWrite":[NSNumber numberWithBool:characteristic.properties & CBCharacteristicPropertyAuthenticatedSignedWrites]
                    }
                }];
            }
            [self callBack:callback data:@{STATUS:STATUS_SUCCESS,
...
- (void)writeBLECharacteristicValue:(NSDictionary *)options callback:(UniModuleKeepAliveCallback)callback {
    __weak typeof(self) weakSelf = self;  // 定义 weakSelf
    if([self isAvailable:callback]){
        NSString *deviceId=[options objectForKey:DEVICE_ID];
        NSString *serviceId=[options objectForKey:SERVICE_ID];
...
- (void)getConnectedBluetoothDevices:(NSDictionary *)options callback:(UniModuleKeepAliveCallback)callback {
    if([self isAvailable:callback]){
        ...
        for(CBPeripheral *dev in arr){
            ...
            NSUInteger mtu = [dev maximumWriteValueLengthForType:CBCharacteristicWriteWithoutResponse];
            
            NSDictionary *d=[self getDeviceData:dev.identifier.UUIDString];
            if(d){
                // 创建一个可变副本来添加新的键值对
                NSMutableDictionary *mutableD = [d mutableCopy];
                mutableD[@"mtu"] = @(mtu);
                [devices addObject:[mutableD copy]]; // 添加不可变的副本
            }else{
                //如果不包含，则可能恢复重连的设备
                if(dev&&dev.name){
