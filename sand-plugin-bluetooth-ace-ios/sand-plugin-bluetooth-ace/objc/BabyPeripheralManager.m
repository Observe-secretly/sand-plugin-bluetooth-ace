...
void makeCharacteristicToService(CBMutableService *service, NSString *UUID, NSString *properties, NSString *descriptor) {
    ...
    if ([properties containsString:@"wr"]) {
        prop |= CBCharacteristicPropertyWriteWithoutResponse;
    }
    if ([properties containsString:@"s"]) {
        prop |= CBCharacteristicPropertyAuthenticatedSignedWrites;
    }
    
    // 如果 properties 是空的，则默认设置为可读和可写
    if (properties == nil || [properties isEqualToString:@""]) {
        prop = CBCharacteristicPropertyRead | CBCharacteristicPropertyWrite;
    }

    // 创建特征
    CBMutableCharacteristic *c = [[CBMutableCharacteristic alloc] initWithType:[CBUUID UUIDWithString:UUID]
                                                                     properties:prop
                                                                          value:nil
    ...
    if (!(descriptor == nil || [descriptor isEqualToString:@""])) {
        ...
        CBMutableDescriptor *desc = [[CBMutableDescriptor alloc] initWithType:CBUUIDCharacteristicUserDescriptionStringUUID
                                                                       value:descriptor];
