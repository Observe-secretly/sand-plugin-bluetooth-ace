...
public class DeviceMirror {
    ...
    private BluetoothGattCallback coreGattCallback = new BluetoothGattCallback() {
            ...
            if (newState == BluetoothGatt.STATE_CONNECTED) {
                ...
                if (connectCallback != null) {
                    ...
                    if (status == BluetoothGatt.GATT_SUCCESS) {
                        ...
                        connectCallback.onDisconnect(bluetoothLeDevice,isActiveDisconnect);
                    } else {
                        connectState = ConnectState.CONNECT_FAILURE;
                        connectCallback.onConnectFailure(bluetoothLeDevice,new ConnectException(gatt, status));
                    }
                }
        ...
        public void onMtuChanged(BluetoothGatt gatt, int mtu, int status) {
            ...
            if (status == BluetoothGatt.GATT_SUCCESS) {
                Log.d(TAG, "MTU size changed: " + mtu);
                deviceMirror.mtu = mtu;
            } else {
                Log.d(TAG, "MTU size change failed with status: " + status);
                deviceMirror.mtu=20;
            }
            deviceMirror.getBluetoothLeDevice().setMTU(deviceMirror.mtu);
            //更新镜像设备信息
            Log.d(TAG, "onMtuChanged更新MTU到镜像");
            ViseBle.getInstance().getDeviceMirrorPool().updateDeviceMirror(deviceMirror);
        }
        ...
        public void onServicesDiscovered(final BluetoothGatt gatt, final int status) {
            ...
            if (status == 0) {
                ...
                if (connectCallback != null) {
                    ...
                    bluetoothGatt.requestMtu(bluetoothLeDevice.getMTU());
                    connectCallback.onConnectSuccess(deviceMirror);
                }

                // 现在GATT连接应该是稳定的，可以请求MTU
                Log.d(TAG, "GATT连接稳定.更新MTU到镜像");
        ...
        public void onReadRemoteRssi(BluetoothGatt gatt, int rssi, int status) {
            ...
            if (status == BluetoothGatt.GATT_SUCCESS) {
                if (rssiCallback != null) {
                    rssiCallback.onSuccess(bluetoothLeDevice,rssi);
                }
            } else {
                if (rssiCallback != null) {
                    rssiCallback.onFailure(bluetoothLeDevice,new GattException(status));
                }
            }
        }
    };
    ...
    public void writeData(byte[] data) {
        if (data == null || data.length > bluetoothLeDevice.getMTU()) {
            ViseLog.e("this data is null or length overflows.");
            return;
        }
     ...
     * 设置MTU
     */
    public synchronized void requestMtu(int mtuSize) {
        if (bluetoothGatt != null) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
    ...
    private synchronized void connect() {
        ...
        if (bluetoothLeDevice != null && bluetoothLeDevice.getDevice() != null) {
            bluetoothLeDevice.getDevice().connectGatt(ViseBle.getInstance().getContext(), bluetoothLeDevice.isAutoReconnect(), coreGattCallback);
        }
    }
    ...
    private void connectFailure(BleException bleException) {
        if (connectRetryCount < BleConfig.getInstance().getConnectRetryCount()) {
            ...
            close();
            if (connectCallback != null) {
                connectCallback.onConnectFailure(bluetoothLeDevice,bleException);
            }
