...
public class SandPluginBluetoothAce extends UniModule {
        ...
        if(this.isAvailable(callback)){
            ...
            if(characteristics!=null){
                ...
                for(BluetoothGattCharacteristic characteristic : characteristics){
                    ...
                    pro.put("writeNoResponse",(characteristic.getProperties()&BluetoothGattCharacteristic.PROPERTY_WRITE_NO_RESPONSE)!=0x0);
                    pro.put("signedWrite",(characteristic.getProperties()&BluetoothGattCharacteristic.PROPERTY_SIGNED_WRITE)!=0x0);
            ...
            if(list!=null){
                for(DeviceMirror dm : list){
                    if(dm.getConnectState()==ConnectState.CONNECT_SUCCESS){
                        //这里放弃从缓存获取，缓存数据更新存在问题，只能渲染外围设备列表
                        //JSONObject d= this.getDeviceData(dm.getBluetoothLeDevice().getAddress());
                        BluetoothLeDevice bleInfo = dm.getBluetoothLeDevice();
                        JSONObject data = convertBluetoothLeDeviceJson(bleInfo);
                        if(data!=null){
                            devices.add(data);
                        }
                    }
    ...
    private void initListener() {
        ...
        //监听设备各种事件
        SandPluginBluetoothAce _this=this;
        //监听蓝牙适配器状态
    ...
    private JSONObject convertBluetoothLeDeviceJson(BluetoothLeDevice device){
        String localName;
    ...
    private void updateDevice(BluetoothLeDevice device){
        ...
        JSONObject data = convertBluetoothLeDeviceJson(device);
